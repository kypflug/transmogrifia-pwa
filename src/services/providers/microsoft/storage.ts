 ...
 *
 * Implements the StorageProvider interface from ../types.ts using
 * Microsoft Graph API and OneDrive AppFolder storage.
 */
...
// ─── Shared utilities (module-level exports) ────────────────────────
...
export class OneDriveStorageProvider implements StorageProvider {
  ...
  private extractDeletedArticleId(item: Record<string, unknown>): string | null {
    ...
    for (let i = 0; i < items.length; i += METADATA_CONCURRENCY) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (item) => {
          ...
          return this.downloadMeta(item.id, headers);
        }),
      );
  ...
  async syncArticles(): Promise<DeltaSyncResult> {
    ...
    if (isFullSync) {
      const indexMetas = await this.downloadIndex(headers);
      if (indexMetas !== null) {
      ...
      while (url) {
        ...
        for (const item of items) {
          ...
          if (item.deleted || item['@removed']) {
            const deletedId = this.extractDeletedArticleId(item);
    ...
    if (toDownload.length > 0) {
      const { metas, failureCount, deletedDuringDownload } = await this.downloadMetaBatch(toDownload, headers);
      upserted.push(...metas);
    ...
    if (finalDeltaLink) {
      if (!hasDownloadFailures) {
        await this.saveDeltaToken(finalDeltaLink);
      } else {
  ...
  async listArticles(): Promise<OneDriveArticleMeta[]> {
    ...
    const { metas } = await this.downloadMetaBatch(toDownload, headers);
    return metas;
  }
  ...
  async uploadMeta(
    ...
    if (res.status === 412 && mergeFn) {
      ...
      try {
        const remoteMeta = await this.downloadMeta(meta.id, headers);
  ...
  async downloadArticleAsset(drivePath: string): Promise<Blob> {
    const headers = await this.authHeaders();
    const res = await fetch(
  ...
  async rebuildIndex(articles: OneDriveArticleMeta[]): Promise<void> {
    try {
      const headers = await this.authHeaders();
      await this.uploadIndex(articles, headers);
    } catch (err) {
  ...
  /** Check whether a delta token exists (used by divergence detection). */
   ...
   */
  async uploadSettings(envelope: SyncEncryptedEnvelope, updatedAt: number, syncVersion?: number): Promise<void> {
    const headers = await this.authHeaders();
...
export function createOneDriveStorage(getAccessToken: () => Promise<string>): OneDriveStorageProvider {
  return new OneDriveStorageProvider(getAccessToken);
}
